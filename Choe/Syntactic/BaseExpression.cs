...
namespace Choe.Syntactic
{
    public abstract class BaseExpression
    {
        ...
        public virtual BaseExpression Integrate(string vName)
        {
            ...
            BaseExpression formulaExpr = FormulaTranslator.CheckExpression(expr, vName);
            if(formulaExpr != null)
            {
                return formulaExpr;
            }
        ...
        public static BaseExpression Build(ExpressionContext context, string value, Dictionary<string, string> matchingVariables)
        {
            if (string.IsNullOrEmpty(value))
                return (BaseExpression)null;
            string str = Parser.RemoveSurrounders(value, Elements.FunctionBrackets, Elements.Brackets);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (checkedData2.Is)
                    {//return null;
                        var buildExpr = BaseExpression.Build(context, checkedData2.Value.Operand, matchingVariables);
                        if (buildExpr == null) return null;
                        else
                            baseExpression = (BaseExpression)new UnaryOperatorExpression(value, checkedData2.Value.Operator, pos1, buildExpr);
                    }
                    else
                    {
                        ...
                        if (checkedData3.Is)
                        {
                            BaseExpression anOperand = BaseExpression.Build(context, checkedData3.Value.Operand, matchingVariables);
                            if (anOperand == null)
                                return null;
                            OperatorPosition pos2 = OperatorPosition.Outfix;
                            baseExpression = (BaseExpression)new UnaryOperatorExpression(value, checkedData3.Value.Operator, pos2, anOperand);
        ...
        public static List<string> GetLiteralsAndVariables(ExpressionContext context, string value)
        {
            if (string.IsNullOrEmpty(value))
                return null;
